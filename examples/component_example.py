...
def main() -> None:
    """Run script."""
    output_dir = pathlib.Path("component_example")
    ...
    # Convert both to an mchammer molecule.
    ...
    # And create from components a supramolecule.
    ...
    for i, comp in enumerate(list(conformer.get_components())):
        if i == 0:
            ...
            new_cage.write(output_dir / "comp_new_host.mol")
        elif i == 1:
            new_guest = stk_guests[0].with_position_matrix(
                comp.get_position_matrix()
            )
            new_guest.write(output_dir / "comp_new_guest.mol")
