...
def main() -> None:
    """Run script."""
    output_dir = pathlib.Path("custom_potential_example")
    ...
    class PotFn(spd.SpdPotential):
        ...
        def __init__(
            self, guest_scale: float, nonbond_epsilon: float = 5
        ) -> None:
    ...
    class CentroidFn:
        ...
        Here we show that as long as you provide a `compute_potential` method,
        then any class works!
        ...
        def compute_potential(self, supramolecule: spd.SupraMolecule) -> float:
            """Compute the potential."""
            centroids = [
                i.get_centroid() for i in supramolecule.get_components()
            ]
    ...
    opt_host_guest.write(output_dir / "custom_pot_fn_out.mol")
